...
public class AiBehaviour : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        player.OnBark.AddListener(Activate);
        
    }

    void Activate()
    {
        //If optimization becomes an issue, change 'magnitude' to 'sqrmagnitude', and multiply the float by itself. saves an apparently pretty powerful 'exponent' calculation
        if((bird.transform.position - playerLocation.position).magnitude < 5.0f)
        {
            on = true;
            if (animator)
